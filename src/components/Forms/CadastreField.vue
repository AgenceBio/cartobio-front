<template>
  <div class="horizontal-stack">
    ...
    <div class="fr-input-group">
      <label
        :for="`parcel-number-${fieldId}`"
        class="fr-label"
        aria-label="Numéro de parcelle"
      >
        N° de parcelle
    ...
    <div class="fr-input-group fr-input-group--actions">
      <span
        :class="{'fr-icon': true, 'fr-icon-check-line': hasFeature && !$props.helpText.error, 'fr-icon-alert-fill': doesNotExist, 'fr-icon-more-fill': isFetchingGeometry }"
        :disabled="!hasFeature"
      />
    </div>
  </div>
  <span
    :class="{ 'fr-hint-text': !$props.helpText.error, 'fr-error-text': $props.helpText.error }"
