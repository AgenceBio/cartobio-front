<template>
  <div>
    ...
    <p>
      Vous pouvez importer un fichier géographique exporté depuis CartoBio, le
      <a href="https://www.geoportail.gouv.fr/" target="_blank" rel="noopener noreferrer">Geoportail<lien-externe /></a>
      ou encore <a href="https://qgis.org" target="_blank" rel="noopener noreferrer">QGIS<lien-externe /></a>.
    </p>

    <p>
      Les formats acceptés sont
      <abbr>KML</abbr> (<code>.kml</code> ou <code>.kmz</code>), GeoJSON (<code>.geojson</code>), GeoPackage
      (<code>.gpkg</code>) et Shapefile (<code>.shp</code> ou <code>.zip</code>).
    </p>
    ...
    <div class="fr-upload-group fr-mb-5w">
      <input
        type="file"
        ref="fileInput"
        accept=".geojson,.gpkg,.json,.kml,.kmz,.shp,.shp.zip,.zip"
