<template>
  ...
  <button
    v-if="showControl"
    type="button"
    :aria-pressed="isAscending || isDescending"
    :id="id"
    class="fr-btn fr-btn--tertiary-no-outline"
    @click="emit('update:modelValue', { sort: code, order: !isAscending ? 'asc' : 'desc' })"
    :aria-label="`Trier par ${$slots.default()[0].children.toLocaleLowerCase()}`"
  >
    <svg width="14" height="16" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M7 16L1 10L13 10L7 16Z"
        :fill="isDescending ? '#000091' : '#929292'"
      />
    </svg>
  </button>
</template>
