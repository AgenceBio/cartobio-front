<template>
  ...
  >
    <div @click="pressZoom(feature.id)" class="fr-grid-row parcelle-titre">
      <h4 class="fr-text--lg">{{ featureName(feature) }}</h4>
      <div class="fr-grid-row parcelle-actions">
        <template v-if="isGroupedByCulture">
          <small v-if="feature.properties.cultures.length > 1">Multi-culture</small>
        </template>
        <template v-else>
          <span v-if="feature.properties.cultures.length > 1">
          ...
          <span v-else>{{ cultureLabel(feature.properties.cultures[0]) }}</span>
        </template>
      ...
      <div class="fr-grid-row fr-grid-row--middle gap-10">
        ...
        <ActionDropdown with-icons>
          ...
          <li v-else>
            <button
              type="button"
              class="fr-btn fr-btn--tertiary-no-outline ri-eye-line ri-xl fr-text--sm"
