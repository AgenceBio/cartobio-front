    ...
    <form @submit.prevent="">
      <article class="fr-pl-5w fr-mb-5w">
        <div class="fr-radio-group fr-ml-n4w fr-mb-2w">
          ...
          <label class="fr-label" for="radio-source-cadastre"> Par référence cadastrale </label>
        </div>
        ...
        <div class="fr-mb-10v" v-for="({ commune, key }, index) in cadastreParcelles" :key="key">
          ...
          <div class="fr-input-group">
            <label class="fr-label"> Reference cadastrale </label>
            <CadastreField
              :commune="flowSource === 'cadastre' ? commune : ''"
        ...
        <span v-if="multipolygon" class="fr-hint-text fr-message--error fr-mb-3v">
          Les références cadastrales renseignées sont disjointes. Afin de pouvoir ajouter cette parcelle, les références
          cadastrales doivent se toucher.
        </span>
      ...
      <article class="fr-pl-5w fr-mb-5w">
        <div class="fr-radio-group fr-ml-n4w fr-mb-2w">
          ...
          <label class="fr-label" for="radio-source-dessin"> Par dessin sur la carte </label>
        </div>
      </article>
      ...
      <article class="fr-pl-5w fr-mb-5w">
        <div class="fr-radio-group fr-ml-n4w fr-mb-2w">
          ...
          <label class="fr-label" for="radio-source-telepac"> Par référence Telepac (prochainement) </label>
        </div>
      </article>
...
import CadastreField from "@/components/forms/fields/CadastreField.vue";
...
import OperatorEditForm from "@/components/forms/SingleItemOperatorForm.vue";
import { statsPush } from "@/stats.js";
import toast from "@/utils/toast.js";
import { diff, featureName, inHa, legalProjectionSurface, merge } from "@/utils/features.js";
...
// Cadastre references
const cadastreParcelles = reactive([
  { commune: "", reference: "", feature: null, error: "", key: crypto.randomUUID() },
]);
const feature = ref(null);
const canAddReference = computed(
  () => cadastreParcelles.length > 0 && cadastreParcelles.every((p) => p.feature !== null),
);
...
watch(cadastreParcelles, () => {
  ...
  multipolygon.value =
    combinedFeature.geometry.type === "MultiPolygon" && combinedFeature.geometry.coordinates.length > 1;
...
function nextButton() {
  ...
  if (flowSource.value === "dessin") {
    return router.push(
      `/exploitations/${operatorStore.operator.numeroBio}/${recordStore.record.record_id}/ajout-parcelle/dessin`,
    );
  }
}
