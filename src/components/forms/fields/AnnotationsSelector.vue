<template>
  <div class="fr-input-group">
    ...
    <ul class="fr-tags-group fr-tags-group--annotations fr-my-2w">
      <li
        :class="`annotation-choice annotation--${annotationId}`"
        v-for="(annotation, annotationId) in visibleAnnotations"
        :key="annotationId"
      >
        <button
          class="fr-tag"
  ...
  <div class="fr-input-group" v-if="isSelected(ANNOTATIONS.REDUCED_CONVERSION_PERIOD)">
    ...
    <select
      class="fr-select"
      name="reduced_conversion_period_state"
  ...
  <div class="fr-alert fr-alert--info fr-my-2w" v-if="isSelected(ANNOTATIONS.DOWNGRADED) && featureId">
    <p>
      Voici l'identifiant de parcelle à renseigner dans l'API&nbsp;Contrôle de l'Agence Bio :
      <code>{{ featureId }}</code
      >.
    </p>
  </div>
...
const reducedConversionStates = computed(
  () => AnnotationTags[ANNOTATIONS.REDUCED_CONVERSION_PERIOD].metadata[ANNOTATIONS.METADATA_STATE]
);
